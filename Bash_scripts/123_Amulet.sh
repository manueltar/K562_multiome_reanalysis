...
do
    ...
    myjobid_Amulet_run=$(sbatch --job-name $name_Amulet_run --output=$outfile_Amulet_run --partition=cpuq --time=24:00:00 --nodes=1 --ntasks-per-node=$processors --mem-per-cpu=$mem --parsable --wrap="Rscript $Rscript_Amulet_run --sample_name $sample_name --frag_file $frag_file --barcode_file $barcode_file --repeats $repeats --type $type --out $output_dir --processors $processors --total_memory $total_memory")
