...
do
    ...
    myjobid_Seurat_first_pass=$(sbatch --job-name $name_Seurat_first_pass --output=$outfile_Seurat_first_pass --partition=cpuq --time=24:00:00 --nodes=1 --ntasks-per-node=$processors --mem-per-cpu=$mem --parsable --wrap="Rscript $Rscript_Seurat_first_pass --sample_name $sample_name --master_path $master_path --rna_min_features $rna_min_features --atac_min_fragments $atac_min_fragments --MITO_max $MITO_max --type $type --out $output_dir --processors $processors --total_memory $total_memory")
