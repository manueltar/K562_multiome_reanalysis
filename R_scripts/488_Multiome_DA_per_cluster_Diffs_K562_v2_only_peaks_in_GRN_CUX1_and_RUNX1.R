...
DA_function = function(option_list)
{
  ...
  GRN_peaks<-read.table(file=opt$GRN_peaks, sep="\t", header=TRUE)
  ...
  Peak_list_filt=Peak_list %>% dplyr::filter(peak_ids %in% GRN_peaks$Peak_ID)
...
main = function() {
  ...
  option_list <- list(
    ...
    make_option(c("--Diff_sel"), type="character", default=NULL, 
                metavar="type", 
                help="Path to tab-separated input file listing regions to analyze. Required."),
    make_option(c("--GRN_peaks"), type="character", default=NULL, 
                metavar="type", 
                help="Path to tab-separated input file listing regions to analyze. Required."),
