...
read_all_barcoded_samples = function(option_list)
{
    ...
    if(dim(df_files_sel)[1] >0)
    {
      ...
      if(SIZE_gate> 0)
      {
        ...
        if(LINE_gate> 0)
        {
          ...
          results_no_duplicated_UMIS_collapsed_UMIS.dt<-data.table(results_no_duplicated_UMIS_collapsed_UMIS, key=c("sample","CellBC"))
          
          
          
          Final_table<-as.data.frame(results_no_duplicated_UMIS_collapsed_UMIS.dt[,.(GFPbc_string=paste(GFPbc, collapse = ';'),
                                                                                                                   GFPbc_attribution=length(unlist(strsplit(paste(GFPbc, collapse = ';'), split=';'))),
                                                                                                                   Number_of_UMIS_string=paste(Number_of_UMIS, collapse = ';')), 
                                                                                                                by=key(results_no_duplicated_UMIS_collapsed_UMIS.dt)], stringsAsFactors=F)
  ...
  if(length(list_results) >0)
  {
    ...
    Results_uniquely_genotyped<-Results[which(Results$GFPbc_attribution == Threshold_attributed_genotypes),]
    
    cat("Results_uniquely_genotyped_0\n")
    cat(str(Results_uniquely_genotyped))
    cat("\n")
    cat(sprintf(as.character(names(summary(Results_uniquely_genotyped$GFPbc_attribution)))))
    cat("\n")
    cat(sprintf(as.character(summary(Results_uniquely_genotyped$GFPbc_attribution))))
    cat("\n")
    
    colnames(Results_uniquely_genotyped)[which(colnames(Results_uniquely_genotyped) == 'Number_of_UMIS_string')]<-'Number_of_UMIS'
    colnames(Results_uniquely_genotyped)[which(colnames(Results_uniquely_genotyped) == 'GFPbc_string')]<-'GFPbc'
    
    Results_uniquely_genotyped$Number_of_UMIS<-as.integer(Results_uniquely_genotyped$Number_of_UMIS)
    
    Results_uniquely_genotyped$sample<-factor(Results_uniquely_genotyped$sample,
                                              levels=samples,
                                              ordered=T)
    Results_uniquely_genotyped$GFPbc<-factor(Results_uniquely_genotyped$GFPbc,
                                             levels=genotypes_string,
                                             ordered=T)
    
    Results_uniquely_genotyped<-Results_uniquely_genotyped[order(Results_uniquely_genotyped$sample,Results_uniquely_genotyped$GFPbc),]
    
    
    cat("Results_uniquely_genotyped_1\n")
    cat(str(Results_uniquely_genotyped))
    cat("\n")
    ...
    Results_uniquely_genotyped_PASS_filter_reads<-Results_uniquely_genotyped[which(Results_uniquely_genotyped$Number_of_UMIS >= Threshold_UMIS_per_cell),]
    
    cat("Results_uniquely_genotyped_PASS_filter_reads_0\n")
    cat(str(Results_uniquely_genotyped_PASS_filter_reads))
    cat("\n")
    cat(sprintf(as.character(names(summary(Results_uniquely_genotyped_PASS_filter_reads$Number_of_UMIS)))))
    cat("\n")
    cat(sprintf(as.character(summary(Results_uniquely_genotyped_PASS_filter_reads$Number_of_UMIS))))
    cat("\n")
    cat(sprintf(as.character(names(summary(Results_uniquely_genotyped_PASS_filter_reads$GFPbc)))))
    cat("\n")
    cat(sprintf(as.character(summary(Results_uniquely_genotyped_PASS_filter_reads$GFPbc))))
    cat("\n")
    ...
    write.table(Results_uniquely_genotyped_PASS_filter_reads, file='Uniquely_genotyped_and_filtered_larry_barcodes_assignments.csv', sep=",",quote=F,row.names=F)
    
    saveRDS(Results_uniquely_genotyped, file='Uniquely_genotyped_larry_barcodes_assignments.rds')
...
graph_function = function(option_list)
{
  ...
  Results_uniquely_genotyped<-readRDS(file='Uniquely_genotyped_larry_barcodes_assignments.rds')
  ...
  Threshold_array<-seq(1,Threshold_UMIS_per_cell, by=1)
